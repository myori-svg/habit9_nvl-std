...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={handleCopy} className="btn-ghost" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, padding: '6px 12px' }}>
...
function PromptBox({ prompt, label }: { prompt: string; label?: string }) {
  return (
    <div style={{ marginBottom: 16 }}>
      {label && <div style={{ fontSize: 11, letterSpacing: '0.07em', textTransform: 'uppercase', color: 'var(--ink-soft)', marginBottom: 6 }}>{label}</div>}
      <div style={{ position: 'relative' }}>
        <pre style={{ background: 'var(--parchment)', border: '1px solid var(--border)', padding: '14px 16px', fontSize: 12, lineHeight: 1.7, whiteSpace: 'pre-wrap', wordBreak: 'break-word', margin: 0, maxHeight: 300, overflow: 'auto', fontFamily: 'DM Sans, sans-serif' }}>
          {prompt}
        </pre>
...
function SaveResultBox({ novel, selectedPartId, selectedDQId }: { novel: Novel; selectedPartId: string; selectedDQId: string }) {
  ...
  return (
    <div style={{ marginTop: 20, padding: 16, background: 'var(--parchment)', border: '1px solid var(--border)' }}>
      <div style={{ fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--ink-soft)', marginBottom: 10 }}>Gemini 결과 저장하기</div>
      <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
        {(['composition', 'charInfo', 'charPrompt'] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)} style={{ padding: '4px 10px', fontSize: 11, cursor: 'pointer', background: tab === t ? 'var(--ink)' : 'white', color: tab === t ? 'var(--parchment)' : 'var(--ink-soft)', border: '1px solid', borderColor: tab === t ? 'var(--ink)' : 'var(--border)' }}>
      ...
      <textarea className="input-field" value={value} onChange={(e) => setValue(e.target.value)} placeholder="Gemini에서 생성된 결과를 여기에 붙여넣기" style={{ fontSize: 12, minHeight: 80, marginBottom: 8 }} />
...
프롬프트 텍스트만 출력. 영어로 작성.`;
  ...
  const selectedDQ = scenePart?.discussionQuestions.find((d) => d.id === sceneDQId);
  const selectedChars = novel.characters.filter((c) => sceneCharIds.includes(c.id));
  const charPromptsText = selectedChars.map((c) => `{${c.name}}: ${c.textPrompt || '(텍스트 프롬프트 없음 — ④ 단계에서 생성 필요)'}`).join('\n\n');
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      <div style={{ marginBottom: 24 }}>
        <h2 className="serif" style={{ fontSize: 26, fontWeight: 300, margin: '0 0 6px' }}>Manual Mode — {novel.title}</h2>
        <p style={{ fontSize: 13, color: 'var(--ink-soft)', margin: 0, opacity: 0.7 }}>각 단계의 프롬프트를 복사해서 Gemini에 직접 붙여넣고, 결과를 다시 저장할 수 있어요.</p>
      </div>

      {/* Step tabs */}
      <div style={{ display: 'flex', gap: 6, marginBottom: 20, flexWrap: 'wrap' }}>
        {STEPS.map((step) => (
          <button key={step.id} onClick={() => setActiveStep(step.id as ActiveStep)} style={{ padding: '8px 14px', cursor: 'pointer', fontSize: 12, fontWeight: 500, background: activeStep === step.id ? 'var(--ink)' : 'white', color: activeStep === step.id ? 'var(--parchment)' : 'var(--ink-soft)', border: '1px solid', borderColor: activeStep === step.id ? 'var(--ink)' : 'var(--border)', transition: 'all 0.15s' }}>
            {step.label}
          </button>
        ))}
      </div>
      ...
      <div className="card" style={{ padding: 24 }}>
        ...
        {activeStep === 'dq' && (
          <div>
            {parsedChapters.length > 0 ? (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 11, color: 'var(--ink-soft)', marginBottom: 8, letterSpacing: '0.07em', textTransform: 'uppercase' }}>챕터 선택 (복수 선택 가능)</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
                  {parsedChapters.map((c) => {
                    const sel = dqChapters.includes(c.label);
                    return (
                      <div key={c.label} onClick={() => setDqChapters((prev) => sel ? prev.filter((x) => x !== c.label) : [...prev, c.label])} style={{ padding: '5px 12px', border: '1px solid', cursor: 'pointer', fontSize: 12, borderColor: sel ? 'var(--gold)' : 'var(--border)', background: sel ? 'rgba(201,168,76,0.1)' : 'white', display: 'flex', alignItems: 'center', gap: 5, transition: 'all 0.15s' }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <textarea className="input-field" value={dqCustomSummary} onChange={(e) => setDqCustomSummary(e.target.value)} placeholder="사용할 챕터 서머리를 붙여넣기" style={{ fontSize: 12, minHeight: 100 }} />
              </div>
            )}
        ...
        {activeStep === 'composition' && (
          <div>
            {novel.parts.length > 0 ? (
              <div style={{ marginBottom: 16 }}>
                <select value={compPartId} onChange={(e) => setCompPartId(e.target.value)} className="input-field" style={{ flex: 1, fontSize: 12 }}>
                  {novel.parts.map((p) => <option key={p.id} value={p.id}>{p.label}</option>)}
                </select>
              </div>
            ) : (
              <textarea className="input-field" value={compCustomDQ} onChange={(e) => setCompCustomDQ(e.target.value)} placeholder="Discussion Question을 입력하세요" style={{ fontSize: 12, minHeight: 60, marginBottom: 16 }} />
            )}
        ...
        {activeStep === 'char-prompt' && (
          <div>
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              {novel.characters.length > 0 ? (
                <select value={charPromptName} onChange={(e) => {
            ...
            <textarea className="input-field" value={charPromptInfo} onChange={(e) => setCharPromptInfo(e.target.value)} placeholder="캐릭터 정보 (③에서 Gemini가 생성한 결과 붙여넣기)" style={{ fontSize: 12, minHeight: 80, marginBottom: 12 }} />
        ...
        {activeStep === 'scene' && (
          <div>
            {novel.parts.length > 0 && (
              <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                <select value={scenePartId} onChange={(e) => { setScenePartId(e.target.value); setSceneDQId(''); setSceneComposition(''); }} className="input-field" style={{ flex: 1, fontSize: 12 }}>
                  {novel.parts.map((p) => <option key={p.id} value={p.id}>{p.label}</option>)}
                </select>
                <select value={sceneDQId} onChange={(e) => {
                  ...
                  // Auto-detect characters from composition
            ...
            <textarea className="input-field" value={sceneComposition} onChange={(e) => {
              setSceneComposition(e.target.value);
              const matches = e.target.value.match(/\{([^}]+)\}/g)?.map((m) => m.slice(1, -1)) ?? [];
              const autoIds = novel.characters.filter((c) => matches.some((m) => m.toLowerCase() === c.name.toLowerCase())).map((c) => c.id);
              if (autoIds.length > 0) setSceneCharIds(autoIds);
            }} placeholder="구도 프롬프트 (②에서 생성한 결과 붙여넣기, 또는 DQ 선택 시 자동 입력)" style={{ fontSize: 12, minHeight: 80, marginBottom: 12 }} />
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {novel.characters.map((c) => {
                    ...
                    return (
                      <div key={c.id} onClick={() => setSceneCharIds((prev) => sel ? prev.filter((x) => x !== c.id) : [...prev, c.id])} style={{ padding: '5px 12px', border: '1px solid', cursor: 'pointer', fontSize: 12, borderColor: sel ? 'var(--gold)' : 'var(--border)', background: sel ? 'rgba(201,168,76,0.1)' : 'white', display: 'flex', alignItems: 'center', gap: 5 }}>
                        {sel && <Check size={10} style={{ color: 'var(--gold)' }} />}
                        {c.name}
