...
const STEPS = [
  ...
  { id: 'char-prompt', label: '④ 캐릭터 프롬프트', desc: '이미지 생성용 텍스트 프롬프트 작성' },
...
function SaveResultBox({ novel, selectedPartId, selectedDQId }: { novel: Novel; selectedPartId: string; selectedDQId: string }) {
  const { updateDQ, updateCharacter } = useStore();
  const [tab, setTab] = useState<'composition' | 'charInfo' | 'charPrompt'>('composition');
    ...
    <div style={{ marginTop: 20, padding: 16, background: 'var(--parchment)', border: '1px solid var(--border)' }}>
      ...
      {(tab === 'charInfo' || tab === 'charPrompt') && novel.characters.length > 0 && (
        <select value={charName} onChange={(e) => setCharName(e.target.value)}
          className="input-field" style={{ fontSize: 12, marginBottom: 8 }}>
          {novel.characters.map((c) => <option key={c.id} value={c.name}>{c.name}</option>)}
        </select>
      )}
      <textarea className="input-field" value={value} onChange={(e) => setValue(e.target.value)}
        placeholder="Gemini에서 생성된 결과를 여기에 붙여넣기"
        style={{ fontSize: 12, minHeight: 80, marginBottom: 8 }} />
      <button type="button" className="btn-primary" onClick={handleSave} disabled={!value.trim()}
...
export default function ManualPanel({ novel }: Props) {
  ...
  const [charPromptInfo, setCharPromptInfo] = useState(novel.characters[0]?.info ?? '');

  // ── Step 5: Char Image
  const [charImageName, setCharImageName] = useState(novel.characters[0]?.name ?? '');
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [imagePreview, setImagePreview] = useState('');
  const [uploading, setUploading] = useState(false);
  const [uploadDone, setUploadDone] = useState(false);
  const fileRef = useRef<HTMLInputElement>(null);
  const { updateCharacter, saveCharImage, updateNovel } = useStore();
  ...
  const selectedSummary = parsedChapters.length > 0
    ? parsedChapters.filter((c) => dqChapters.includes(c.label)).map((c) => `${c.label}\n${c.content}`).join('\n\n')
    : dqCustomSummary;
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        setUploadDone(true);
        setTimeout(() => { setUploadDone(false); setImageFile(null); setImagePreview(''); }, 1500);
      };
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 20, flexWrap: 'wrap' }}>
        {STEPS.map((step) => (
          <button type="button" key={step.id} onClick={() => setActiveStep(step.id as ActiveStep)} style={{
      ...
      <div className="card" style={{ padding: 24 }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
                  {parsedChapters.map((c) => {
                    ...
                    return (
                      <div key={c.label} onClick={() => setDqChapters((prev) => sel ? prev.filter((x) => x !== c.label) : [...prev, c.label])}
                        style={{ padding: '5px 12px', border: '1px solid', cursor: 'pointer', fontSize: 12, borderColor: sel ? 'var(--gold)' : 'var(--border)', background: sel ? 'rgba(201,168,76,0.1)' : 'white', display: 'flex', alignItems: 'center', gap: 5, transition: 'all 0.15s' }}>
                ...
                {dqChapters.length > 0 && (
                  <div style={{ padding: '10px 12px', background: 'var(--parchment)', border: '1px solid var(--border)', fontSize: 12, color: 'var(--ink-soft)', maxHeight: 120, overflow: 'auto' }}>
                    <pre style={{ margin: 0, whiteSpace: 'pre-wrap', fontFamily: 'DM Sans, sans-serif', fontSize: 12 }}>{selectedSummary}</pre>
                  </div>
                )}
              </div>
            ) : (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 11, color: 'var(--ink-soft)', marginBottom: 6 }}>챕터가 자동 감지되지 않았어요. 직접 입력하세요:</div>
                <textarea className="input-field" value={dqCustomSummary} onChange={(e) => setDqCustomSummary(e.target.value)}
                  placeholder="사용할 챕터 서머리를 붙여넣기" style={{ fontSize: 12, minHeight: 100 }} />
              </div>
            )}
        ...
        {activeStep === 'composition' && (
          <div>
            {novel.parts.length > 0 ? (
              <div style={{ marginBottom: 16 }}>
                <select value={compPartId} onChange={(e) => setCompPartId(e.target.value)}
                  className="input-field" style={{ flex: 1, fontSize: 12 }}>
                  {novel.parts.map((p) => <option key={p.id} value={p.id}>{p.label}</option>)}
                </select>
              </div>
            ) : (
              <textarea className="input-field" value={compCustomDQ} onChange={(e) => setCompCustomDQ(e.target.value)}
                placeholder="Discussion Question을 입력하세요" style={{ fontSize: 12, minHeight: 60, marginBottom: 16 }} />
            )}
            <PromptBox prompt={compositionPrompt} label="Gemini에 붙여넣을 프롬프트" />
          </div>
        )}
          ...
          <div>
            <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
              {(['extract', 'info'] as const).map((t) => (
                <button type="button" key={t} onClick={() => setCharSubStep(t)} style={{
            ...
            {charSubStep === 'extract' && (
              <div>
                ...
                <textarea className="input-field" value={charExtractInput}
                  onChange={(e) => {
                    setCharExtractInput(e.target.value);
                    const matches = e.target.value.match(/\{([^}]+)\}/g)?.map((m) => m.slice(1, -1)) ?? [];
                    const unique = matches.filter((name, idx) => matches.indexOf(name) === idx);
                  ...
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      {extractedChars.map((name) => {
                        ...
                        return (
                          <div key={name} style={{ padding: '5px 12px', fontSize: 12, border: '1px solid', borderColor: exists ? 'var(--sage)' : 'var(--gold)', background: exists ? 'rgba(74,103,65,0.08)' : 'rgba(201,168,76,0.1)', display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <button type="button" className="btn-primary"
                    onClick={() => {
                      const newChars = extractedChars
                        ...
                        .map((name) => ({ id: crypto.randomUUID(), name, info: '', textPrompt: '', createdAt: new Date().toISOString() }));
                      updateNovel(novel.id, { characters: [...novel.characters, ...newChars] });
                    }}
                    style={{ fontSize: 12, padding: '7px 16px', display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
                  ...
                  {novel.characters.map((c) => {
                    ...
                    return (
                      <div key={c.id} onClick={() => { if (hasInfo) return; setCharInfoNames((prev) => selected ? prev.filter((n) => n !== c.name) : [...prev, c.name]); }}
                        style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', border: '1px solid', borderColor: selected ? 'var(--gold)' : 'var(--border)', background: hasInfo ? 'var(--parchment)' : selected ? 'rgba(201,168,76,0.08)' : 'white', cursor: hasInfo ? 'not-allowed' : 'pointer', opacity: hasInfo ? 0.5 : 1, transition: 'all 0.15s' }}>
                ...
                {charInfoPrompts.length > 0 && (
                  <PromptBox label="Gemini에 붙여넣을 프롬프트"
                    prompt={charInfoPrompts.map(({ name, prompt }) => `[${name}]\n${prompt}`).join('\n\n===\n\n')} />
                )}
              </div>
            )}
          </div>
        ...
        {activeStep === 'char-prompt' && (
          <div>
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              {novel.characters.length > 0 ? (
                <select value={charPromptName} onChange={(e) => { setCharPromptName(e.target.value); const c = novel.characters.find((ch) => ch.name === e.target.value); setCharPromptInfo(c?.info ?? ''); }}
                  className="input-field" style={{ fontSize: 12, flex: 1 }}>
                  {novel.characters.map((c) => <option key={c.id} value={c.name}>{c.name}</option>)}
                </select>
              ) : (
                <input className="input-field" value={charPromptName} onChange={(e) => setCharPromptName(e.target.value)} placeholder="캐릭터 이름" style={{ fontSize: 12, flex: 1 }} />
              )}
            </div>
            <textarea className="input-field" value={charPromptInfo} onChange={(e) => setCharPromptInfo(e.target.value)}
              placeholder="캐릭터 정보 (③에서 Gemini가 생성한 결과 붙여넣기)" style={{ fontSize: 12, minHeight: 80, marginBottom: 12 }} />
        ...
        {activeStep === 'char-image' && (
          <div>
            ...
            {novel.characters.length > 0 ? (
              <select value={charImageName} onChange={(e) => setCharImageName(e.target.value)}
                className="input-field" style={{ fontSize: 12, marginBottom: 16 }}>
                {novel.characters.map((c) => (
              ...
              <p style={{ fontSize: 12, color: 'var(--ink-soft)', opacity: 0.5, marginBottom: 16 }}>③-0에서 먼저 캐릭터를 추출해주세요</p>
            )}
            ...
            <div onClick={() => fileRef.current?.click()} style={{ border: '2px dashed var(--border)', padding: '24px', textAlign: 'center', cursor: 'pointer', marginBottom: 12, background: 'white', transition: 'border-color 0.2s' }}
              ...
              {imagePreview ? (
                ...
                <span style={{ fontSize: 12, color: 'var(--ink-soft)' }}>클릭해서 이미지 선택</span>
              )}
            </div>

            <button type="button" className="btn-primary" onClick={handleImageUpload}
              disabled={!imageFile || uploading || novel.characters.length === 0}
              style={{ fontSize: 12, padding: '7px 16px', display: 'flex', alignItems: 'center', gap: 6 }}>
              {uploadDone ? <><Check size={12} /> 저장됨!</> : uploading ? '업로드 중…' : 'Firebase에 저장'}
            </button>
          </div>
        )}
        ...
        {activeStep === 'scene' && (
          <div>
            {novel.parts.length > 0 && (
              <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                <select value={scenePartId} onChange={(e) => { setScenePartId(e.target.value); setSceneDQId(''); setSceneComposition(''); }}
                  className="input-field" style={{ flex: 1, fontSize: 12 }}>
                  {novel.parts.map((p) => <option key={p.id} value={p.id}>{p.label}</option>)}
                </select>
                <select value={sceneDQId} onChange={(e) => {
                  setSceneDQId(e.target.value);
                  const dq = scenePart?.discussionQuestions.find((d) => d.id === e.target.value);
                  if (dq?.compositionPrompt) setSceneComposition(dq.compositionPrompt);
                  if (dq?.compositionPrompt) {
                    const matches = dq.compositionPrompt.match(/\{([^}]+)\}/g)?.map((m) => m.slice(1, -1)) ?? [];
                    const autoIds = novel.characters.filter((c) => matches.some((m) => m.toLowerCase() === c.name.toLowerCase())).map((c) => c.id);
                    if (autoIds.length > 0) setSceneCharIds(autoIds);
                  }
                }} className="input-field" style={{ flex: 2, fontSize: 12 }}>
            ...
            <textarea className="input-field" value={sceneComposition}
              onChange={(e) => {
            ...
            {novel.characters.length > 0 && (
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 11, color: 'var(--ink-soft)', marginBottom: 6, letterSpacing: '0.07em', textTransform: 'uppercase' }}>등장 캐릭터</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {novel.characters.map((c) => {
                    const sel = sceneCharIds.includes(c.id);
                    return (
                      <div key={c.id} onClick={() => setSceneCharIds((prev) => sel ? prev.filter((x) => x !== c.id) : [...prev, c.id])}
                        style={{ padding: '5px 12px', border: '1px solid', cursor: 'pointer', fontSize: 12, borderColor: sel ? 'var(--gold)' : 'var(--border)', background: sel ? 'rgba(201,168,76,0.1)' : 'white', display: 'flex', alignItems: 'center', gap: 5 }}>
                        {sel && <Check size={10} style={{ color: 'var(--gold)' }} />}
                        {c.name}
            ...
            <PromptBox prompt={scenePrompt} label="Gemini에 붙여넣을 최종 장면 생성 프롬프트" />
            <SaveResultBox novel={novel} selectedPartId={scenePartId} selectedDQId={sceneDQId} />
          </div>
        )}
