...
export function buildDQPrompt(
	...
	return `소설 "${novelTitle}"의 챕터별 서머리를 보고 각 챕터에 맞는 Discussion Question을 생성해주세요.
...
export function buildCompositionPrompt(questionItems: string): string {
	return `각 <항목>별로 어울리는 배경화면을 생성할 수 있도록 화풍, 캐릭터 외형을 제외한 장면의 구도를 나타내는 이미지 생성 프롬프트를 생성해줘
...
export function buildCharInfoPrompt(novelTitle: string, name: string): string {
	return `소설 "${novelTitle}"에 등장하는 캐릭터 "${name}"의 정보를 정리해주세요.
...
export function buildCharPromptPrompt(
	...
	return `아래 조건에 따라 캐릭터의 외형을 잘 드러나는 이미지 텍스트 프롬프트를 생성해주세요.
...
export function buildCharImagePrompt(
	...
	return `캐릭터 ${charImageName}의 이미지를 생성하기 위한 프롬프트를 작성해주세요.
  - Full-body storybook illustration
...
export function buildScenePrompt(
	...
	return `아래 지시 사항에 따라 이미지를 생성해주세요.

- 스타일은 <image style>을 따를 것${hasStyleImage ? " (스타일 참고 이미지도 함께 제공)" : ""}
